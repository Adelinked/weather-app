...
const OneDay = ({ date, weather, max, min }: oneDayData) => {
  const { system, pending } = useWeatherStore((state) => state);
...
export const Details = ({ initialData }: { initialData: forcastDataType }) => {
  const { forcastData } = useWeatherStore((state) => state);
...
const Humidity = ({ humidity }: { humidity: number }) => {
  const { pending } = useWeatherStore((state) => state);
    ...
    >
      ...
      {!pending ? (
        <>
          ...
          <div
            className={`w-[229px] h-[8px] rounded-[80px] ${humidityBarClass}`}
          >
            <div
